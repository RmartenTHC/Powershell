...
foreach ($Computer in $ComputerName) {
  if(Test-Connection -ComputerName $Computer -Count 1 -ea 0) {
   try {
    $Networks = Get-WmiObject Win32_NetworkAdapterConfiguration -ComputerName $Computer -EA Stop -Credential $admin| ? {$_.IPEnabled}
   } catch {
   ...
   foreach ($Network in $Networks) {
    ...
    $OutputObj | Add-Member -MemberType NoteProperty -Name InterfaceIndex -Value $InterfaceIndex
    $OutputObj | Add-Member -MemberType NoteProperty -Name Description -Value $Description
    $allips += $OutputObj
    #$allips
    $allips|Export-Csv -NoTypeInformation servernic1.csv -Append -Force

   }
  }
}
#$allips|Export-Csv -NoTypeInformation servernictest.csv
}
